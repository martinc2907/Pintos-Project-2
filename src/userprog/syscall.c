...
static void check_user_pointer(struct intr_frame *f, void * user_ptr){
	...
	/* Mapping relationship: user-adress <-> physical address <-> kernel virtual address 
		this way, kernel has access to user addresses too. */
	//Doesn't thread_current->pagedir check kernel page directory??????????
